...
////////////////////////////////////////////
/** 将ts 入口文件路径目录解析成 entry map 只读取一级目录 * */
var entry = helpers.getEntry('src', 'typescript', 'pages');

entry["polyfills"] = path.join(__dirname,"src", "typescript", "polyfills.ts");
...
var plugins = [
	...
	new HtmlWebpackPlugin({
		...
		title: 'index title',
		name: 'index',
		template: './tpl/template.ftl',
		filename: 'index.html',
		showErrors: false,
		chunks: ['polyfills','core', 'index'],
		envProfile: 'dev',
		//	inject:false,
		inject: 'body',
		minify: false
...
module.exports = {
	...
	module: {
		loaders: [{
			 ...
			 * moduleId: module.id, 无法被webpack解析,遂移除 'webpack-replace?{search: "moduleId: module.id,",replace: ""}'
			 * 也可以先用uglify 压缩，再用replace移除
			 * 
			 * */
			test: /\.ts$/,
			loaders: ['webpack-replace?{search: "moduleId: module.id,",replace: ""}', 'awesome-typescript-loader', 'angular2-template-loader', 'webpack-replace?{search: ".css!",replace: ".css"}']
		}, {
			test: /\.html$/,
			loader: 'html'
		}, {
